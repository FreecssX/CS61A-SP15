...
def find_closest(location, centroids):
    ...
    for i in range(len(centroids)):
        ...
        if i == 0:
            min = dis
        if min > distance(location, centroids[i]):
            min = dis
            mindex = i
    return centroids[mindex]
...
def find_centroid(restaurants):
    ...
    for r in restaurants:
        loc1 += restaurant_location(r)[0]
        loc2 += restaurant_location(r)[1]
    return [loc1 / len(restaurants), loc2 / len(restaurants)]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    best_p = max([predictor for predictor in predictors], key = lambda x: x[1])
    return best_p[0]
